...
c167(str_contains($ci, 'file-24-sanitized-source-snapshot-exact-head') || (preg_match('/file-24-sanitized-source-snapshot-cycle([0-9]+)/', $ci, $artifactMatch) === 1 && (int) ($artifactMatch[1] ?? 0) >= 167), 'Sanitized source artifact naming must remain monotonic or use exact-head naming.');
