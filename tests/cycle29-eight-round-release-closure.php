...
$assert(str_contains($plugin, 'Version:     0.25.9'), 'Plugin header must expose Foundation 0.25.9.');
$assert(str_contains($plugin, "define('SPCRC_VERSION', '0.25.9')"), 'Runtime constant must expose Foundation 0.25.9.');
$assert(str_contains($readme, 'Stable tag: 0.25.9'), 'WordPress readme must expose the 0.25.9 stable tag.');
$assert(($sbom['packages'][0]['versionInfo'] ?? '') === '0.25.9', 'SPDX package version must match 0.25.9.');
$assert(str_contains($licenses, 'License Inventory — 0.25.9'), 'License inventory must match 0.25.9.');
$assert(str_contains($registry, 'release:file-24-0.25.9'), 'Self manifest evidence source must match 0.25.9.');
...
$assert(str_contains($manifest, 'Foundation eight-round corrective candidate 0.25.9'), 'Source manifest must identify the current candidate.');
