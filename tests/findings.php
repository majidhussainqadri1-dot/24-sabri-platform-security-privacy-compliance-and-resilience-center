...
function current_time(string $type, bool $gmt = false): string { return '2026-07-31 02:30:00'; }
...
final class FindingWpdb
{
    ...
    {
        if (is_array($query)) {
            $uuid = (string) ($query['args'][0] ?? '');
            return $this->findings[$uuid]['status'] ?? null;
        }
        return count(array_filter($this->findings, static fn (array $row): bool => in_array($row['status'] ?? '', ['open', 'triaged', 'in-progress'], true)));
    }
...
expectFinding($repository->openCount() === 1, 'Open finding count must include active findings.');

$status = $repository->setStatus($uuid, 'resolved');
expectFinding($status === true, 'Allowed finding status must update.');
expectFinding($repository->openCount() === 0, 'Resolved findings must leave the open count.');
expectFinding(is_wp_error($repository->setStatus($uuid, 'deleted')), 'Unknown finding status must fail closed.');
expectFinding(is_wp_error($repository->setStatus('11111111-1111-4111-8111-111111111111', 'resolved')), 'Unknown finding UUID must not report success.');
...
echo "PASS: security finding lifecycle\n";
