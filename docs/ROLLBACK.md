...
8. Inspect `spcrc_upgrade_lock`. Remove it only when its owner is proven absent/stale and the action is recorded; never clear an active migration lock to force an upgrade.
9. Do not retry privacy work merely because older code cannot interpret newer evidence.
10. Do not treat an older package's inability to evaluate 0.25.7 governance binding as approval. Re-open accepted risks/findings or keep them blocked until the current governed package is restored.
11. Preserve every independently keyed governance/audit gap. Reconcile only through the current authorized workflow after returning to 0.25.7 or a later compatible release.
